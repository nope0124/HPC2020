...
//            }
        ...
        if (distance <= can && id + 1 < warn) {
            id++;
            tx = path[id] % W;
            ty = path[id] / W;
            distance = sqrt((aRabbitPos.x - (tx + 0.5)) * (aRabbitPos.x - (tx + 0.5)) + (aRabbitPos.y - (ty + 0.5)) * (aRabbitPos.y - (ty + 0.5)));
            dirX = (can * (tx + 0.5) + (distance - can) * aRabbitPos.x) / distance;
            dirY = (can * (ty + 0.5) + (distance - can) * aRabbitPos.y) / distance;
            IdirX = dirX;
            IdirY = dirY;
        }
